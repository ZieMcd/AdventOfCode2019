...
namespace Part_1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (string pat in path1) // each instruction is stored in pat 
            {
                length1 = Convert.ToInt32(pat.Substring(1, pat.Length-1));//  seeing how long path is for current instruction ie. L120, length is 120
              

                for (int i = 0; i < length1; i++) // this  loop is used to map current instruction on to loop
                {

                    // if state ments are used to check what direction 
                    if (pat[0] == 'U')    
                    {

                        if (matrix2[currentPos[0], currentPos[1]] < k && matrix2[currentPos[0], currentPos[1]] != 0)
                        {
                            break;
                        }
                        else
                        {
                            matrix1[currentPos[0], currentPos[1]] = k;
                        }
            ...
            //------------------------MAPPING PATH TWO--------------------------//
            foreach (string pat in path2)
            {
                length2 = Convert.ToInt32(pat.Substring(1, pat.Length - 1));
                ...
                for (int i = 0; i < length2; i++)
                {
                    if (pat[0] == 'U')
                    {
                        ...
                        {
                            matrix2[currentPos[0], currentPos[1]] = k;
                        }
            ...
            double distance, lowDis;
            lowDis =10000000;

        //---------------------------SEARCHING ARRAYS-----------------------------//

        /*Searching both arrays to see where they both =1 
         * then finding the shortest distance
         */
        ...
        for (int i =0; i < 40000; i++)
            {
                ...
                {
                    if (matrix1[i,j] > 0 && matrix2[i, j] > 0 && matrix2[i, j] != 0 && matrix1[i, j] != 0)
                    {

                        distance = matrix1[i, j] + matrix2[i, j];
                       ...
                       /* distance = Math.Abs(20000-j) + Math.Abs(20000-i);
                        if (distance == 0)
                        {
                            break;
